...
import com.tappybird.entities.Bird
import com.tappybird.entities.Pipe
import com.tappybird.systems.DifficultyManager
import com.tappybird.utils.SaveManager

class GameScreen(
        private val game: TappyBirdGame,
    ...
    private val pipes = com.badlogic.gdx.utils.Array<Pipe>()
    private val typingQueue = game.typingQueue
    ...
    private val mainMenuItems = listOf("Resume", "Difficulty", "Audio", "Main Menu")
    ...
    private var nextPipeInterval = diffManager.pipeInterval // Initialize with default
    private var score = 0
    ...
    private var level = 1
    private var totalPipesSpawned = 0
    ...
    init {
        ...
        giraffeObstacles = Array(4) { i -> game.assetManager.get("assets/giraffe_long_$i.png") }
        ...
        val distanceToSpawn = diffManager.scrollSpeed * nextPipeInterval
    ...
    override fun show() {
        ...
        game.soundManager.currentTrack = com.tappybird.systems.SoundManager.MusicTrack.WHAT
    ...
    private fun update(delta: Float) {
        ...
        // Pipe Spawning
        pipeTimer += delta
        if (!isPracticeMode && pipeTimer >= nextPipeInterval) {
            pipeTimer = 0f
            // Calculate next interval with variance
            val base = diffManager.pipeInterval
            // Random variance between 0.7 (70%) and 1.3 (130%)
            val variance = 0.7f + Math.random().toFloat() * 0.6f
            nextPipeInterval = base * variance

            // Spawn Pipe offscreen
            ...
            // Pass the screen height to pipe if needed, or just pipe handles it?
            // Pipe is just x,y,gap,width.
            // X should be worldWidth + some buffer
            val isSnake = Math.random() < 0.125 // 1/8 chance
            pipes.add(Pipe(viewport.worldWidth + 50f, gapY, isSnake))
            totalPipesSpawned++
        }
        ...
        while (iterator.hasNext()) {
            ...
            pipe.update(delta, diffManager.scrollSpeed)
            ...
            if (!pipe.collided &&
                            (Intersector.overlaps(bird.bounds, pipe.bottomBounds) ||
                                    Intersector.overlaps(bird.bounds, pipe.topBounds))
            ) {
                if (!isPracticeMode) {
                    // Pipe collision is now non-fatal
                    pipe.collided = true
                    soundManager.playCrash()
        ...
        if (bird.bounds.y <= 60) { // Screen height for ground is 60
            if (isPracticeMode) {
                // Bounce/Slide logic
                bird.y = 60f + (bird.bounds.height - 50f) / 2f + 20f // Place slightly above
                bird.velocity = 0f
            } else {
                endGame()
            }
    ...
    private fun softReset() {
        ...
        // Reset Pipe Logic for determinstic spawn
        pipeTimer = 0f
        nextPipeInterval = diffManager.pipeInterval
    ...
    private fun draw(delta: Float) {
        ...
        // Draw Pipes with Heads
        for (pipe in pipes) {
            val texture =
                    if (pipe.isSnake) {
                        anacondaObstacles[pipe.headIndex % 2]
                    } else {
                        giraffeObstacles[pipe.headIndex]
                    }
            pipe.render(game.batch, texture)
        }
        ...
        if (flashTimer > 0) {
            val scale =
                    1.0f + 0.6f * kotlin.math.sin(flashTimer * 10f).let { if (it < 0) -it else it }
            barWidth *= scale
            barHeight *= scale
        ...
        // Align both label and value to the same center axis relative to right screen edge
        ...
        if (Gdx.input.isKeyJustPressed(Input.Keys.ENTER) ||
            ...
            if (pauseState == PauseState.MAIN) {
                when (menuSelectedIndex) {
                    0 -> pauseState = PauseState.NONE // Resume
                    1 -> { // Cycle Difficulty
                        difficulty =
                                when (difficulty) {
                    ...
                    3 -> {
                        game.soundManager.stopMusic()
                        game.screen = MenuScreen(game) // Main Menu
                    }
                }
            } else if (pauseState == PauseState.AUDIO) {
